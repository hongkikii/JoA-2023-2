...
public interface RoomInMemberRepository extends JpaRepository<RoomInMember, Long> {
    ...
    Optional<RoomInMember> findByRoomAndMember(@Param("room") Room room, @Param("member") MemberEntity member);

    @Query("SELECT rim FROM RoomInMember rim WHERE rim.member = :member AND rim.expired = '1'")
    List<RoomInMember> findByAllMember(@Param("member") MemberEntity member);
    ...
    Optional<RoomInMember> findByRoomAndExceptMember(@Param("room") Room room, @Param("member") MemberEntity member);
    ...
    List<RoomInMember> checkRoomInMember(@Param("member1") MemberEntity member1, @Param("member2") MemberEntity member2);
    ...
    @Query("SELECT rim.room AS room, rim.room.date AS date, m.name AS name, m.urlCode AS urlCode, mes.content AS content " +
            ...
            "LEFT JOIN MemberEntity m ON rim.member.id = m.id " +
    ...
    Optional<RoomInfoIncludeMessageVO> findRoomInfoIncludeMessage(@Param("room") Room room, @Param("member") MemberEntity member);
    ...
    @Query("SELECT rim.room AS room, rim.room.date AS date, m.name AS name, m.urlCode AS urlCode " +
            ...
            "LEFT JOIN Room r ON rim.room.id = r.id " +
            "WHERE rim.member = :member AND rim.room = :room ")
    Optional<RoomInfoExceptMessageVO> findRoomInfoExceptMessage(@Param("room") Room room, @Param("member") MemberEntity member);

    @Query("SELECT rim FROM RoomInMember rim Where rim.room = :room")
    List<RoomInMember> findAllRoom(@Param("room") Room room);

    @Query("SELECT rim FROM RoomInMember rim Where rim.room = :room AND rim.member <> :member")
    Optional<RoomInMember> findOpponentByRoomAndMember(@Param("room") Room room, @Param("member") MemberEntity member);

    @Query("SELECT rim FROM RoomInMember rim Where rim.room = :room AND rim.member <> :member AND rim.expired = :expired")
    Optional<RoomInMember> checkOpponentExpired(@Param("room") Room room, @Param("member") MemberEntity member, @Param("expired") String expired);

    @Query("SELECT rim.room.id AS roomId ,m.id AS memberId, m.name AS name, m.urlCode AS urlCode, m.bio AS bio " +
            "From RoomInMember rim LEFT JOIN MemberEntity m ON rim.member.id = m.id " +
            "WHERE rim.room = :room AND rim.member <> :member")
    UserInfoVO getUserInfo(@Param("room") Room room, @Param("member") MemberEntity member);
